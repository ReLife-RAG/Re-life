...
export const createCounselorProfile = async (userId: string, data: any) => {
  ...
  if (!user || user.role !== "counselor") throw new Error("User is not a counselor");
  
  const oldProfile = await Counselor.findOne({ userId });
  if (oldProfile) throw new Error("Profile already exists");
  
  if (!data.credentials || data.credentials.length === 0) throw new Error("Need at least one credential");
  if (!data.specializations || data.specializations.length === 0) throw new Error("Need at least one specialization");
...
export const getAllCounselors = async (filters: any = {}) => {
  ...
  if (filters.availability === "true") query["availableSlots.isBooked"] = false;
  ...
  return await Counselor.find(query).populate("userId", "name email").sort(sortOption);
};

export const getCounselorById = async (id: string) => {
  const counselor = await Counselor.findById(id).populate("userId", "name email").populate("reviews.UserId", "name");
  if (!counselor) throw new Error("Counselor not found");
  return counselor;
};

export const updateCounselorProfile = async (counselorId: string, userId: string, updates: any) => {
  const counselor = await Counselor.findById(counselorId);
  if (!counselor) throw new Error("Profile not found");
  if (counselor.userId.toString() !== userId) throw new Error("You cannot edit this profile");
  if (updates.specializations && updates.specializations.length === 0) throw new Error("Need at least one specialization");
  if (updates.credentials && updates.credentials.length === 0) throw new Error("Need at least one credential");
  if (updates.price && updates.price < 0) throw new Error("Price cannot be negative");
...
export const createBooking = async (userId: string, data: any) => {
  ...
  const slot = counselor.availableSlots.find((s: any) => !s.isBooked && s.startTime === sessionTime);
...
export const getUserSessions = async (userId: string, status?: string) => {
  ...
  return await Session.find(query).populate("counselorId").populate("userId", "name email").sort({ sessionDate: 1 });
};
...
export const getCounselorSessions = async (userId: string, status?: string) => {
  ...
  if (status) query.status = status.toUpperCase();
  return await Session.find(query).populate("userId", "name email").populate("counselorId").sort({ sessionDate: 1 });
};
...
export const cancelSession = async (sessionId: string, userId: string, reason?: string) => {
  ...
  if (!isUser && !isCounselor) throw new Error("You are not part of this session");
  if (session.status === "COMPLETED") throw new Error("Cannot cancel completed session");
  ...
  if (counselor) {
    const slot = counselor.availableSlots.find((s: any) => s.startTime === session.sessionTime.toTimeString().substring(0, 5));
...
export const completeSession = async (sessionId: string, userId: string, notes?: string) => {
  const session = await Session.findById(sessionId);
  if (!session) throw new Error("Session not found");

  const counselor = await Counselor.findById(session.counselorId);
  if (!counselor || counselor.userId.toString() !== userId) throw new Error("Only counselor can complete session");
